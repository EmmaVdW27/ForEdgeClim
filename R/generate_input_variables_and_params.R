...
#' @export
create_input_data <- function(datetime) {

  # Save all input variables as global variables (<<-)
  # input files
  TLS_input_file <<- 'Data/2023-07-10_ForSe_Gontrode_5cm_transect_emma.las'
  TLS_filtered_file <<- 'Data/TLS_scaled_DTM_and_grid_July.rds'
  DTS_input_file <<- read.csv(paste0("Data/DTS_unfiltered/", format(datetime, "%Y-%m-%d"), "_hourly.csv"))
  ...
  infl_macro <<- 55       # Distance over which the influence of macro temp on air temp is reduced by 50% (m)
  infl_soil <<- 10         # Distance over which the influence of soil temp on air temp is reduced by 50% (m)
